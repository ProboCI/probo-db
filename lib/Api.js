...
class Api {
  ...
  addRoutes() {
    ...
    /*self.server.post('/merge/:type', checkTypeMiddleware, function(req, res, next) {
      // Creates or updates an object
      // use for update, use for create, use for createOrUpdate, use for assign
      self.mergeObject(req, res, next);
    });

    self.server.post('/delete/:type', checkTypeMiddleware, function(req, res, next) {
      // Deletes an object.
      // This should be a safe delete by default (use for safeDelete)
  ...
  getObject(req, res, next) {
    ...
    // @todo: we need to get a schema definition
    // or some sort of orm here
    // this actually works for everything except build
    // i think we should switch the column name on the build table
    self.db.getObjects(type, {buildId: id}, {getOne: true})
      .then(function(result) {
        res.send(result);
        next();
      })
